...
export default function Page() {
  ...
  return (
    <>
      ...
      {
        hydration ? <div className='py-10 relative'>
          <div className='text-main'>
            <Linebg/>
          </div>
            <Link href={"/home"}>
          <div className='bg-main  text-body flex  items-center fixed px-2 py-2  rounded-full left-24 gap-x-1 group'>
          ...
          <div className=' mx-4  lg:mx-auto lg:w-1/2  rounded-pixel py-2 px-4 lg:px-8  relative  z-1 bg-blury  mBlur Gborder '>
            {
              info?.fullName && bill?.length != 0 ? <div className=''>
                ...
                <div className='my-4 flex justify-center '>
                  <Link href="/checkout/status" >
                    <button onClick={() => checkOut()} className='text-body  font-semibold    w-52 hover:w-64 bg-blury  mBlur Gborder  rounded-pixel px-3 py-1 my-3  transition-all hover:px-8 hover:shadow-xl'> Complete Order <IoCheckmarkDoneCircle className='inline' /></button>
                  </Link>
                </div>
        ...
        </div> : <div className='min-h-screen flex items-center'><Loading shape="box" /></div>
      }
